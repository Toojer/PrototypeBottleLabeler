...
LabelerStates PeelSticker(bool stickerSensor, LabelerStates PreviousState);

//This will signal the sticker is peeled and conveyor belt will start turning to label a bottle.
LabelerStates LabelBottle(bool stickerSensor, LabelerStates PreviousState);

//When no stimulus received that a new bottle is present, the conveyor belt will continue to roll out the bottles.
LabelerStates ExitBottle(AccelStepper &stepper, bool labelTrigger, LabelerStates PreviousState);
